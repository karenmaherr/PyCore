# PyCore

A personal collection of Python projects covering **Python Fundamentals**, **Machine Learning**, **Deep Learning**, and **Agentic AI** practice. This repo serves as a hands-on portfolio and playground for applying concepts learned through coursework and self-study.

##  About

PyCore is where I build, break, and rebuild things while learning ML/DL/Agentic AI. Each project is self-contained and focused on a specific concept, technique, or tool.

##  Repository Structure

```
pycore/
├── machine-learning/     # Classical ML projects (regression, classification, clustering, etc.)
├── deep-learning/        # Neural networks, CNNs, computer vision, etc.
├── agentic-ai/           # LLM agents, tool use, multi-step reasoning projects
└── README.md
```

> Structure will evolve as more projects are added.

##  Topics Covered

- **Machine Learning**: data preprocessing, feature engineering, model building & evaluation
- **Deep Learning**: neural networks, computer vision
- **Agentic AI**: building and experimenting with AI agents

##  Tech Stack

- Python
- Common ML/DL libraries (NumPy, Pandas, Scikit-learn, etc.)
- Jupyter Notebooks for experimentation

##  Getting Started

Each project folder includes its own code and (where relevant) a brief explanation of what it does and what was learned.

##  Purpose

This repo is a living log of my learning journey in AI/ML — not a production codebase. Expect experiments, iterations, and continuous updates as I grow my skills and portfolio.

